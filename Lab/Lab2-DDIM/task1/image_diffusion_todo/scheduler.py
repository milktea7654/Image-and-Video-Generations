...
class BaseScheduler(nn.Module):
    def __init__(
        ...
        if mode == "linear":
            ...
            t = torch.linspace(0, T, steps=T + 1, dtype=torch.float64) / T  
...
class DDPMScheduler(BaseScheduler):
    ...
    def step_predict_noise(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        device = x_t.device
        dtype = x_t.dtype
        beta_t = self.betas[t].to(device=device, dtype=dtype)
        alpha_t = self.alphas[t].to(device=device, dtype=dtype)
        alpha_bar_t = self.alphas_cumprod[t].to(device=device, dtype=dtype)
        alpha_bar_prev = (self.alphas_cumprod[t - 1].to(device=device, dtype=dtype)
                        if t > 0 else torch.tensor(1.0, device=device, dtype=dtype))
        mean = (x_t - (beta_t / torch.sqrt(1.0 - alpha_bar_t)) * eps_theta) / torch.sqrt(alpha_t)
    ...
    def step_predict_x0(self, x_t: torch.Tensor, t: int, x0_pred: torch.Tensor):
        ...
        eps_pred = (x_t - alpha_bar_t.sqrt() * x0_pred) / (1 - alpha_bar_t).sqrt()
        ...
        if t > 0:
            ...
            noise = torch.randn_like(x_t)
            sample_prev = mean + variance.sqrt() * noise
        else:
            sample_prev = mean
    ...
    def step_predict_mean(self, x_t: torch.Tensor, t: int, mean_theta: torch.Tensor):
        ...
        if t > 0:
            ...
            variance = beta_t * (1 - alpha_bar_t_prev) / (1 - alpha_bar_t)
            noise = torch.randn_like(x_t)
            sample_prev = mean_theta + variance.sqrt() * noise
        else:
...
class DDIMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor, predictor: str):
        ...
        if t_prev < 0:
            alpha_bar_t_prev = torch.tensor(1.0, dtype=alpha_bar_t.dtype, device=alpha_bar_t.device)
        else:
        ...
        x_t_prev_det = torch.sqrt(alpha_bar_t_prev) * x0_pred + torch.sqrt(1 - alpha_bar_t_prev - sigma_t**2) * eps_theta
        if self.eta > 0 and t_prev >= 0:
            noise = torch.randn_like(x_t)
            sample_prev = x_t_prev_det + sigma_t * noise
        else:
            sample_prev = x_t_prev_det
