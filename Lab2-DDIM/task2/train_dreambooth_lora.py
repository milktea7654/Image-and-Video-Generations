...
def log_validation(
    ...
    # Save validation images to local directory
    validation_dir = os.path.join(args.output_dir, "validation_images")
    os.makedirs(validation_dir, exist_ok=True)
    
    phase_name = "test" if is_final_validation else "validation"
    for i, image in enumerate(images):
        image_path = os.path.join(validation_dir, f"{phase_name}_epoch_{epoch}_image_{i}.png")
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter only valid image files, excluding metadata.jsonl and other non-image files
        valid_extensions = {'.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'}
        all_files = list(Path(instance_data_root).iterdir())
        self.instance_images_path = [f for f in all_files if f.suffix.lower() in valid_extensions]
